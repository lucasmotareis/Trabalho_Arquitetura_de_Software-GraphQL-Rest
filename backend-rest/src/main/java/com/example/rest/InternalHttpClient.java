...
public class InternalHttpClient {
    ...
    private void record(
        ...
        double backendMs = 0;
        String nestedTrace = null;
        if (response != null) {
            backendMs = parseDouble(response.getHeaders().getFirst(TimingFilter.BACKEND_TIME_HEADER));
            nestedTrace = response.getHeaders().getFirst(BackendTrace.HEADER);
        }
        ...
        BackendTrace.mergeHeader(nestedTrace, objectMapper);
    }
    ...
    private String extractErrorMessage(String responseText, String fallback) {
        ...
        try {
            ApiTypes.ErrorResponse error = objectMapper.readValue(responseText, ApiTypes.ErrorResponse.class);
